...
const dragonfireProducts = productsData.filter(
  (p) =>
    p.categories?.includes("Dragonfire") || // in category
    p.name?.toLowerCase().includes("dragonfire") // in name
);

// ── Group products dynamically ───────────────────────────────────────
const groupedProducts = dragonfireProducts.reduce((groups, product) => {
  let assigned = false;
