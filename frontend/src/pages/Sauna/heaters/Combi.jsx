...
const GROUP_KEYWORDS = {
  ...
  "Scandia Combi": ["Scandia Combi", "SCAC"], // ✅ Include Scandia Combi
  "Nimbus Combi": ["Nimbus Combi", "NIMC"],
};

// ── Filter Combi products dynamically ─────────────────────────────────
const combiProducts = productsData.filter(
  (p) =>
    p.categories?.includes("Combi") || // normal combi category
    p.name?.toLowerCase().includes("combi") // include "Scandia Combi NS" even if category doesn't include Combi
);

// ── Group products dynamically ───────────────────────────────────────
const groupedProducts = combiProducts.reduce((groups, product) => {
  let assigned = false;
...
// Only display groups in FIXED_ORDER that exist in data
